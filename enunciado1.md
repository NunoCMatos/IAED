...
dígitos decimais, uma paragens de origem e uma paragens de destino.
...
No nome que descreve a paragens podem ocorrer carateres brancos
...
ou seja a origem da ligação é a mesma paragens que a estação terminal da carreira
ou o destino da ligação é a mesma paragens que a estação de partida da carreira.
...
com origem na última paragens de uma carreira e com destino na primeira paragens
...
- __c__ - adiciona e lista as carreiras:
  ...
  - Formato de saída sem argumentos: `<nome-da-carreira> <paragens-origem> <paragens-destino> <número-de-paragens> <custo-total> <duração-total>`, pela ordem de criação. As paragens são omitidas para carreiras sem ligações.
  - Formato de saída com argumentos: `<paragens-origem> { , <paragens> }`, pela sequência do percurso se não for criada uma nova carreira.
...
- __p__ - adiciona e lista paragems:
  - Formato de entrada: `p [ <nome-de-paragens> [ <latitude> <longitude> ] ]`
  - Formato de saída sem argumentos: `<nome-de-paragens>: <latitude> <longitude> <número-de-carreiras>` por cada paragens e pela ordem de criação, uma por linha onde `<número-de-carreiras>` é o número de carreiras que param na paragens.
  - Formato de saída com um argumento: `<latitude> <longitude>`.
    - Se o comando for invocado com três argumentos, é criada uma nova paragens sem gerar dados na saída.
    - Notas:
      - O `<nome-de-paragens>` deve ser delimitado por aspas se o nome contiver carateres brancos (espaço ou tabulador horizontal); o nome não pode conter o carater aspa.
  - Erros:
    - `<nome-de-paragens>: stop already exists.` no caso da criação de uma paragens e já existir uma paragens com o nome indicado.
    - `<nome-de-paragens>: no such stop.` no caso da listagem de uma paragens e não existir uma paragens com o nome indicado.
    - As coordenadas são impressas em 16 posições com 12 dígitos décimais (`%16.12f`).

- __l__ - adiciona ligações:
  - Formato de entrada: `l <nome-de-carreira> <paragens-origem> <paragens-destino> <custo> <duração>`
  - Formato de saída: NADA
  - Notas:
    - O `<nome-de-paragens>` deve ser delimitado por aspas se o nome contiver carateres brancos (espaço ou tabulador horizontal); o nome não pode conter o carater aspa.
  - Erros:
    - `<nome-de-carreira>: no such line.` no caso de não existir uma carreira com o nome indicado.
    - `<nome-de-paragens>: no such stop.` no caso de não existir uma paragens com o nome indicado, origem ou destino.
...
- __i__ - lista as interseções entre as carreiras:
  ...
  - Formato de saída: `<nome-de-paragens> <número-de-carreiras>: <nome-de-carreira> ...` por cada paragens onde pára mais de uma linha, uma por linha por ordem de criação das paragens. Os nomes das carreiras devem ser listados alfabeticamente.
...
O comando `p` com um argumento mostra a latitude e longitude da paragens.

```text
p X1 2.5 45.6
```

O comando `p` com três argumentos cria uma nova paragens.
...
O comando `l` permite adicionar novas ligações a uma carreira. Neste caso, se `X1` for a última paragens da carreira, então a nova ligação é inserida no fim e `A1` é a nova última paragens. Caso contrário, se `A1` é a origem da carreira, então a ligação é inserida no início da carreira e `X1` passa a ser a nova origem. Se ambas as condições se verificarem (inserção de um ciclo na carreira), então a ligação é inserida no fim.
